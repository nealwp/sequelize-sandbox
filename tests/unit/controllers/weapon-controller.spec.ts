...
describe('weapons controller', () => {
    ...
    describe('update', () => {
        test('should throw an error if weapon does not exist', async () => {
            ...
            await expect(async () => await weapons.update(updatedWeapon)).rejects.toThrow(
                `Weapon id ${updatedWeapon.id} not found!`,
            );
        });
    ...
    describe('findById', () => {
        test('should throw an error if inventory does not exist', async () => {
            ...
            await expect(async () => await weapons.findById(weaponId)).rejects.toThrow(
                `Weapon id ${weaponId} not found!`,
            );
        });
    ...
    describe('addToInventory', () => {
        test('should throw an error if weapon is not found', async () => {
            ...
            await expect(async () => await weapons.addToInventory(weaponId, inventoryId)).rejects.toThrow(
                `Weapon id ${weaponId} not found!`,
            );
        });
