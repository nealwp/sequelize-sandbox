...
describe('inventory controller', () => {
    ...
    describe('update', () => {
        test('should throw an error if inventory does not exist', async () => {
            ...
            await expect(async () => await inventory.update(updatedInventory)).rejects.toThrow(
                `Inventory id ${updatedInventory.id} not found!`,
            );
        });
    ...
    describe('findById', () => {
        test('should throw an error if inventory does not exist', async () => {
            ...
            await expect(async () => await inventory.findById(inventoryId)).rejects.toThrow(
                `Inventory id ${inventoryId} not found!`,
            );
        });
